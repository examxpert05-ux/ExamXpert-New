...
import { getTestConfig, getTestConfigByExamName } from '../test-configs.js'
...
export default function DashboardPage() {
    ...
    const getDisplayName = (testName: string) => {
        // First check if it's already a proper exam name (new format)
        const properNames = ['JEE', 'NEET', 'UPSC', 'CAT', 'GATE', 'SSC', 'CLAT', 'NDA', 'NET', 'BANKING', 'IBPS', 'SBI PO', 'NICL AO']
        if (properNames.includes(testName.toUpperCase())) {
            return testName.toUpperCase()
        }

        // If it's a test code (old format), map it to exam name
        const testCodeToName: { [key: string]: string } = {
            't1': 'UPSC',
            't2': 'JEE',
            't3': 'NEET',
            't4': 'CAT',
        ...
        return testCodeToName[testName.toLowerCase()] || testName.toUpperCase()
    }
    ...
    const handleAnalyze = async (test: UserData) => {
        ...
        try {
            // First try to get config by test code (for old tests saved as "t1")
            let testConfig = getTestConfig(test.test_name.toLowerCase())

            // If not found, try to get config by exam name (for new tests saved as "JEE", "UPSC", etc.)
            if (!testConfig) {
                testConfig = getTestConfigByExamName(test.test_name)
            }
